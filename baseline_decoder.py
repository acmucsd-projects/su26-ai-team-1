...
- Image height is fixed at 96px; width varies per sample, padded per-batch.
...
class ImagePositionalEncoding(nn.Module):
    ...
    Why feat_h is knowable at all: input images are a fixed 96px tall (see the
    CONFIRMED block), so the encoder's output height is the constant
    96 // stride, and the variable width is then just (H*W) // feat_h.
    ...
    max_h only needs to cover the encoder's output height (6, confirmed --
    see the CONFIRMED block), but max_w must cover the WIDEST batch: images are
    ~96px tall with variable widths. At stride 16, max_w=64 covers images up
    to 1024px wide.
    """
        ...
        if feat.dim() == 4:
            ...
            if h is None:
                raise ValueError(
                    ...
                    "  ImagePositionalEncoding(d_model, feat_h=96 // stride)\n"
                    "or per call\n"
                    "  img_pos_enc(feat, feat_h=96 // stride)\n"
            ...
            if length % h != 0:
                raise ValueError(
                    ...
                    f"96 // stride) or the encoder is not flattening a full grid."
                )
            w = length // h
...
if __name__ == "__main__":
    ...
    h, w = 6, 12         # h = 96 // stride = 6 (fixed); w varies per batch
